...
| Phase | Scope | Moat |
|-------|-------|------|
| **v2.0: Solo** | Feature + decision CRUD, bootstrap, MCP | Execution speed |
| **v2.1: Team** | Cloud sync, shared knowledge, permissions | Network effects |
| **v2.2: Integrations** | Slack, GitHub PRs, Linear | Distribution lock-in |

### v2.0 Scope (Keep It Simple)
...
**Not in v2.0:** Smart inference, intent classification, magic context selection.
...
```
    ...
    │                    │     │                    │          │                    │
    │  taskwing feature  │     │  project-context   │          │  Vite + React +    │
    │  taskwing decision │     │       tool         │          │  shadcn/ui         │
    │  taskwing bootstrap│     │                    │          │                    │
    └─────────┬──────────┘     └─────────┬──────────┘          └─────────┬──────────┘
              │                          │                               │
              └──────────────────────────┼───────────────────────────────┘
...
```bash
taskwing init                    # Initialize in project
taskwing bootstrap               # Auto-generate from repo (LLM-powered if OPENAI_API_KEY set)
...
│   ├── llm_analyzer.go   # LLM-powered analysis with relationship inference
│   └── git.go            # Git history parsing
├── llm/
│   └── client.go         # Multi-provider ChatModel factory (OpenAI, Ollama)
└── mcp/
    └── context.go        # project-context tool
