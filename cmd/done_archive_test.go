...
func TestDoneCreatesArchiveEntryWithFlags(t *testing.T) {
    ...
    if err := os.WriteFile(cfgPath, []byte(""), 0o644); err != nil { t.Fatalf("write cfg: %v", err) }
    t.Setenv("TASKWING_CONFIG", cfgPath)

    // create a task
    st, err := GetStore()
    if err != nil { t.Fatalf("get store: %v", err) }
    defer st.Close()
    ...
    if err := rootCmd.Execute(); err != nil { t.Fatalf("execute done: %v", err) }
