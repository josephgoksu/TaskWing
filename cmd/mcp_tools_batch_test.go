...
func TestBatchCreate_WithTempIDs_ResolvesDependenciesAndParents(t *testing.T) {
	...
	found := false
	for _, sid := range aTask.SubtaskIDs {
		if sid == childID {
			found = true
			break
		}
	...
	if !found {
		t.Fatalf("A missing child subtask id: %+v", aTask.SubtaskIDs)
	}
}
...
func TestBatchCreate_WithUUIDDependenciesAndParent(t *testing.T) {
	...
	for _, tsk := range all {
		switch tsk.Title {
		case "A depends on existing":
			aID = tsk.ID
		case "B child of existing":
			bID = tsk.ID
		}
	}
	if aID == "" || bID == "" {
		t.Fatalf("missing created tasks a=%s b=%s", aID, bID)
	}
	...
	if !foundDep {
		t.Fatalf("existing task missing dependent %s: %+v", aID, ex.Dependents)
	}
	...
	if !foundChild {
		t.Fatalf("existing missing child subtask id: %+v", ex.SubtaskIDs)
	}
}
