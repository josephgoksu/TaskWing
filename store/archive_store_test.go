...
func TestArchiveStore_CreateListSearchExportImport(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("create from task: %v", err)
	}
	if entry.ID == "" {
	...
	if err := s2.Initialize(map[string]string{"archiveDir": filepath.Join(tmp2, "archive")}); err != nil {
	...
	if len(items2) != 1 {
		t.Fatalf("expected 1 item in s2, got %d", len(items2))
	}
}
...
func TestArchiveStore_Purge_DryRun(t *testing.T) {
	...
	if _, err := s.CreateFromTask(oldTask, "", nil); err != nil {
	...
	if _, err := s.CreateFromTask(newTask, "", nil); err != nil {
	...
	items, _ := s.List()
	if len(items) != 2 {
		t.Fatalf("expected 2 items after dry-run, got %d", len(items))
	}
}
